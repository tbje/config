	  ...
	  (when (and energy-now rate (not (zerop rate))
	    ...
	    (let ((remaining (if (string= charging-state "Discharging")
				 energy-now
                  ...
                  (voltage (string-to-number (match-string 1))))
	      (setq hours (/ (/ (* remaining voltage) rate)
			     10000000.0)))))))


    (list (cons ?c (cond ((> charge-full 0) (> charge-now 0)
	  ...
	  (cons ?B (if charging-state (downcase charging-state)"N/A"))
